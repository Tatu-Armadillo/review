...
begin 
    if not exists (
        ...
        where
            schema_name = 'restaurant_review'
    ) 
    then 
        drop schema if exists restaurant_review cascade; -- REMOVE;
        ...
        create table tables_bookings(
            id_tables_bookings bigserial primary key,
        ...
        alter table tables_bookings add constraint fk_tables_bookings_table foreign key (tables) references tables (id_table);
        alter table tables_bookings add constraint fk_tables_bookings_booking foreign key (booking) references bookings (id_booking);
        alter table reviews add constraint fk_reviews_users foreign key (users) references users (id_user);
